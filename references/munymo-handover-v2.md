...
| Email magic links dying almost immediately | **Fixed (2026-07-29)** — Paul clicked both links in a result email shortly after it arrived and both showed the "expired" fallback. Root cause was **not** expiry: Clerk sign-in tokens are SINGLE USE, and `GET /api/magic` used to 302 straight to Clerk's sign-in URL, so anything that merely *followed* the link spent the token — mail security scanners, link previewers and prefetchers all do this before the human taps. The token was then `accepted` and the real click hit the fallback. Raising the TTL could never have fixed it. Now `GET /api/magic` renders a self-submitting interstitial (`server/_core/magicLinkRedirect.ts`) and only `POST /api/magic` validates and forwards to Clerk; scanners issue GETs, not POSTs. Destination paths are validated (`safeDestination` rejects absolute and protocol-relative URLs — the old handler would happily forward to `//evil.com`) and HTML-escaped. Separately, TTL is now one shared constant: `MAGIC_LINK_TTL_SECONDS` = 48h in `server/_core/magicLink.ts`, replacing two divergent copies (24h in `routers.ts`, **2h** in `scheduledCuration.ts` — the streak-at-risk link could expire before the lockout it was warning about). Covered by `server/magicLink.test.ts`. |
| End-of-day push notifications silently reaching nobody | **Fixed (2026-07-29)** — on 2026-07-29 the result + new-game emails arrived but no push did, with no error anywhere. Root cause: `sendPushToUsers` deletes a subscription row as soon as a push service answers 410/404 (routine — endpoint rotation, PWA update, transient 410), but that does not unsubscribe the *browser*. `usePushNotifications` decided its state purely from `reg.pushManager.getSubscription()`, so the browser still reported "subscribed", the settings toggle still read ON, and every broadcast reached nobody — with no user-visible symptom and no recovery short of manually toggling push off and on. The hook now reconciles against `push.status` and silently re-registers any browser subscription the server has lost (`push.subscribe` is an upsert keyed on endpoint hash, so replaying is harmless). Made visible too: `admin.endOfDay` returns a `pushSummary` (sent/expired/errors per broadcast, plus an explicit note when the cadence guard skips the new-game push), and the curation completion email now reports it — a silent `sent: 0` is no longer indistinguishable from a healthy run. |
| Curation run duration invisible in the completion email | **Fixed (2026-07-29)** — the "✅ Daily curation complete" email reported only the endpoint's own elapsed time ("completed in 8884ms") while the agent had actually been running for close to an hour, making a pathological run look instant. The agent now sends `agentElapsedMs` with its submission and the email reports both, flagging `⚠️ SLOW` past 15 minutes. Per-turn wall-clock and `stop_reason` were added to the agent's turn logs so a slow run can be attributed to one long research turn vs. many turns vs. retry backoff. Note the retry design is *intended* to be slow in the bad case: transient API errors retry in place (30s/60s/120s) and a whole-run retry waits `FULL_RUN_RETRY_DELAY_MS` (10 min) before a second attempt, so ~1h means the first attempt died and the second succeeded — the game was saved by the retry, which is the feature working. |
...
**Magic links in emails:** Every sender goes through `createMagicLink()` in `server/_core/magicLink.ts` (one helper, one TTL — `MAGIC_LINK_TTL_SECONDS`, 48h). It mints a Clerk sign-in token wrapped in a `https://munymo.com/api/magic?token=...&to=...` URL. `GET /api/magic` renders an interstitial that POSTs back to `/api/magic`; only the POST validates the token and forwards to Clerk. Spent or expired tokens land on the custom fallback page instead of Clerk's error screen. **Do not turn the GET back into a redirect** — see the 2026-07-29 row in Section 4 for why.
