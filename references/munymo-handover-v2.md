...
The winner is determined by relative percentage movement from the company's opening price to its closing price on the game date. The company with the higher percentage movement wins, regardless of direction (i.e. a company down 1% beats a company down 3%). **Automated and made canonical 2026-09-17:** `settleFromPrices()` in `server/scoring.ts` recomputes both percentages from the four stored open/close prices inside `closeAndScoreGame` (the single choke point every publish path uses) and overrides the agent's `companyAPerf`/`companyBPerf`/`winnerTicker` if they disagree, logging warnings into the curation summary email. Ties go to Company A. A move over 40% from the supplied prices is treated as a wrong price and the run is refused rather than settled.
...
| Settlement rule enforced server-side (open-to-close, Decision 6) | **Complete (2026-09-17)** — found by the product audit (`references/product-review-2026-09-17.md`): 17 of the last 50 archived games carried a `companyAPerf`/`companyBPerf` that did not match the open→close move computable from the start/end prices printed beside them, and on three days (2026-07-06 NVDA/AVGO, 2026-08-07 NEM/B, 2026-08-11 SPCX/RKLB) the published winner was wrong under the stated rule. Cause: the agent prompt said "opening and closing price and % change" and the reference prompt literally said "from the previous close", so the agent often reported the quote-page daily change (prior-close based, includes the overnight gap that players can see before the 9:30 ET lockout). Fix: (1) `settleFromPrices()` in `server/scoring.ts` derives perf and winner from the four prices whenever all are present, with the agent's figures reduced to warnings; applied inside `closeAndScoreGame` and in both curation handlers' pre-checks, warnings surfaced in the run summary email; (2) both prompts rewritten to state the rule explicitly; (3) `upsertLeaderboardStat` fixed to read the shared threshold (it still hard-coded 20); (4) result card now says "open to close" with the rule stated under the margin, FAQ wording aligned; (5) `scripts/resettle-open-to-close.ts` re-settled history per Paul's decision (re-score, don't annotate): percentages corrected on the mismatched games, the three flipped games re-scored with a settlement note prepended to their result summary, affected players' win/lose streaks rebuilt from history. Tests cover derivation, override, tolerance, tie, implausible-price refusal and the no-prices fallback. |
| AI-curation disclosure + copy-drift fixes from the product audit | **Complete (2026-09-17)** — the landing page called the brief "expert-curated" while it is written by the Claude research agent with no human review, and nothing on the site said so. Now: landing step 2 says "researched fresh each day by Munymo's AI research agent from live, credited sources"; new FAQ entry "Who writes the research?" (also emitted in the FAQPage JSON-LD); a new Disclaimer paragraph states the automated authorship, the credited-source rule, that results settle from prices independently of the text, and where to report errors. Same commit fixes copy the audit found untrue or stale: the landing pill and the leaderboard provisional caption still said 20 games (now read the shared constant); the FAQ said the validation question stays open "even after the next trading day has begun" (it closes when the result publishes after the close); the FAQ said Away Status was "for a defined period" (it is an open-ended toggle, by Decision 5); all four legal pages said "June 2025". |
| Leaderboard seasons + competition engine + bot policy | **Complete (2026-09-17)** — spec in `references/leaderboard-seasons-spec.md`, approved by Paul the same day. The all-time average board was five human-named test bots plus the founder, ranked on a metric whose luck band (±13 points at 10 games) was wider than the gaps between ranks, and it never reset. Now: (1) **season board** (default tab on `/leaderboard`): calendar month of game dates, ranked by TOTAL points via `getSeasonStandings(from, to, memberIds?)` in `server/db.ts` (the competition engine — a league or head-to-head is the same query with a member list) and the pure `rankSeasonStandings` in `shared/leaderboard.ts` (golf ties, percentiles); no qualification gate; past seasons listed with winners; `leaderboard.season` tRPC procedure; (2) **percentiles** in the payload, shown only at ≥ `PERCENTILE_MIN_PLAYERS` (20); (3) **bot policy** in `shared/const.ts`: bot 870002 renamed "Coin Flip" and shown on every board as a flagged benchmark; 870004–870012 hidden from both boards; all six excluded from community stats (`computeAndStoreCommunityStats`), history recomputed by `scripts/apply-bot-policy.ts` (45 of 63 games changed — recent games now honestly show "1 player participated"); tester agent unchanged, reads `TESTER_BOT_IDS`; (4) **all-time tab** keeps the average board and provisional list, with "played X of Y available" (Y = published games since the player's first) beside each average; (5) `dashboard.getStats` returns `season {key,label,points,games,rank,players}` and the dashboard shows a season card beside the all-time one; landing pill and Practice copy updated. No schema change. Next instance (leagues, sponsor events) needs a `competitions` table — schema approval required. |
| Retention loops from the product audit: share card, Gut vs Research, milestones, losing-streak lesson, holiday state, one-click unsubscribe, polish | **Complete (2026-09-17)** — (1) **Share result** (`client/src/components/ShareResultButton.tsx`, on `/game/:id/result` for a signed-in player): one tap builds a Wordle-style block (`Munymo · Sep 16 · LEN vs DHI / 🧠✅ 🔬✅ ⏱20 → 100 pts / 🔥 7-day streak / link`), native share sheet on phones, clipboard elsewhere; no player name. (2) **Gut vs Research** (`shared/insight.ts` `computeGutVsResearch`, fed by `getPlayerPickOutcomes` in `server/db.ts`, returned as `insight` + `insightSummary` from `dashboard.getStats`): instinct-alone vs after-research accuracy, how often research changed the pick and whether it helped or hurt, accuracy by sector (3+ games) and by month — the FAQ claim that was never delivered. New dashboard section. (3) **Milestones** (`shared/milestones.ts`, result page, latest scored game only): first game, qualification day, streak landmarks (5/10/20/30/50/100/200) or a new personal best, win runs (3/5/7/10), perfect games; at most two cards. (4) **Losing-streak intervention** (Decision 2, finally): at 5 wrong in a row the result page links lesson `l500-3` "Checklists Beat Hunches" with a reason; at 3 a gentler nudge to the Hindsight Spotlight. (5) **Holiday state** on `/game` now offers the same practice/archive/lessons cards as every other nothing-live state. (6) **One-click unsubscribe** (`server/unsubscribe.ts`): HMAC-signed `/api/unsubscribe?u=&t=` link in every player email footer plus `List-Unsubscribe` / `List-Unsubscribe-Post` headers (RFC 8058, Gmail/Yahoo bulk-sender rules); GET and POST both flip `users.emailOptIn=false` and render a confirmation page; the /profile toggle turns it back on. Wired through `sendEmail`/`broadcastEmail` and every player call site (result, missed-game, streak-at-risk/first-timer, game-available, welcome). (7) Polish: result page date reads "Wednesday, September 16, 2026"; dashboard dates en-US; "Lose Streak" card hidden at zero; FAQ "morning" framing now "before the US open"; the lockout countdown bar sat 56px above the bottom for signed-out visitors (no tab bar) and now docks to the bottom for them. Demo page still shows five steps to the live game's four — deliberately left: the demo is a walkthrough and the extra granularity helps there. |
| Archive metrics uniform with the game + leaderboard phone layout | **Complete (2026-09-18)** — two issues Paul reported from his iPhone. (1) `/research/:id` stacked all sixteen metrics in one flat column while the live game and practice show them side by side; it now renders the shared `ResearchMetricsPanel` (A beside B, banded The Long Game / Game-Day Setup) plus a chart button per company, so every presentation of the metrics is the same component. `games.getResearch` now returns `chartSnapshot` for published games and the archive `ChartSheet` uses those archived candles (what players saw before they picked); games that predate snapshotting fall back to the live chart, which leaks nothing on a page that already shows the result. (2) The season table pushed Points off the card at phone width: all three leaderboard tables are now `table-fixed` with explicit `<colgroup>` widths (hidden columns hide their `<col>` too), names truncate, the benchmark pill is desktop-only, and the columns hidden on phones (games, average, played-of-available) reappear as a sub-line under the player name; past-season rows wrap. Verified at 375px: no horizontal overflow, table width equals its card. Also closes audit item 22: Paul's own iPhone screenshots show the pages render correctly on a real device. |
| Magic links v3 + self-healing push | **Complete (2026-09-20)** — Paul reported three symptoms he experienced as one: email links dumped him on an auth page, Clerk emailed "new device" every time, and push kept appearing switched off. Findings: (a) v2 links forwarded to Clerk's HOSTED ticket url, which in practice showed the sign-in/sign-up page instead of signing him in; (b) on iPhone an email link always opens in Safari, which shares no storage with the Home Screen app, so "already signed in" was never true there and every manual Google login was a genuinely new session; (c) v2 stamped a flat 24h on every link, so Friday's link for Monday's game was dead by Sunday — an UNUSED link saying "expired"; (d) the DB showed `pushOptIn=true` throughout and FOUR live iPhone subscriptions (Jun 16, Aug 15, Aug 27, Sep 20): the preference was never lost, the app kept failing to find its own subscription, and in Safari the settings card said "Add to Home Screen first", which reads as "off". Fixes: **(1)** `server/_core/magicLink.ts` + `magicLinkRedirect.ts`: the link carries the raw ticket plus purpose (`play`/`result`) and issue game date; `/api/magic` applies Paul's rules — good until used once (Clerk single-use) or until a newer link for the same purpose exists (stateless: superseded when a later game exists for that purpose via `getLatestGameDateForPurpose`), with a 7-day Clerk cap as security backstop — then redirects to OUR `/email-landing` with the ticket in the url FRAGMENT (an inline script in `client/index.html` moves it to sessionStorage and cleans the address bar before GA loads). v2 links already in inboxes still work (ticket read out of the old `u=` wrapper). **(2)** `EmailLanding.tsx`: already signed in → straight to the destination, ticket untouched (no new session, no new-device email, no countdown); otherwise redeems the ticket in place with `signIn.create({strategy:"ticket"})` + `setActive`; failures say which rule applied (used / superseded / week-old / incomplete). Redeeming needs JavaScript, so mail scanners cannot burn the single use. **(3)** Push: `push.status` returns the account-level `optIn`; `usePushNotifications` silently re-creates a missing subscription when the account wants push and permission is already granted; explicit unsubscribe now sets `pushOptIn=false` so the heal never overrides a real "off"; in Safari on iPhone with an existing subscription the settings card says push is ON and delivered to the Home Screen app. **Platform limit (cannot be coded around):** iOS never opens an email link inside an installed web app — only push notification taps do. Push payloads already carry plain paths and open in the signed-in app. |
| Push is the primary channel, email the fallback | **Complete (2026-09-20)** — Paul's call, after the magic-link investigation showed that an installed-app player got every notification twice: a push that opens the app signed in, and an email that (on iPhone, unavoidably) opens Safari signed out. Rule: for each notification, push goes first to every player with `pushOptIn !== false`; `sendPushToUsers` now returns `reachedUserIds` (players with at least one device that ACCEPTED the message — pure helper `reachedUserIds()` in `server/push.ts`, tested); email is then sent only to players push did not reach, still subject to `emailOptIn`. A player whose only subscription bounced is not "reached" and gets the email the same day, so deleting the app never means silence. Applied in all three fan-outs: `admin.endOfDay` (result push now runs BEFORE the email loop; skipped players don't get magic links minted either), `admin.activateGame` (gained a push it never had), and `streakAtRiskHandler` — the streak-at-risk and finish-your-pick reminders were EMAIL-ONLY until now, so push users never got the two nudges that do the most retention work; they now go by push (`munymo-streak-<id>` / `munymo-finish-<id>` tags, url `/game`) with email fallback, and a player with email off but push on is no longer skipped. Welcome email unchanged (sent before anyone has push). Profile copy explains the email toggle is now the backup channel. **Deliberately not built:** a "send me both" option — it needs a third preference state, i.e. a new `users` column, which requires Paul's written schema approval. Judgment calls Paul delegated (2026-09-20): result emails are not kept as an inbox record for push users; revisit with real player data. |
...
| Practice mode — archived games (server) | **Server complete (2026-09-16), migration 0016 applied.** New `practice_picks` table — deliberately a SEPARATE TABLE, not an `isPractice` flag on `player_picks`: that table feeds `getPlayerScoreHistory` → `upsertLeaderboardStat` → the ranked leaderboard, and every one of those queries would have needed a filter. One forgotten WHERE clause and practice silently pollutes the competitive board, which is the one thing this feature must never do; separation makes it impossible by construction rather than by vigilance. Scores are stored ON the practice row, never in `daily_scores`, so nothing reading that table can mistake practice for live play. Practice never touches the leaderboard, streaks, or qualification. **Blind play enforced SERVER-side** (`practice.getGame`), not in the client — hiding fields in the UI would be no protection since the payload would still carry the answer: `winner`, both `%` moves, the **gameDate** and the dated source article are withheld until that player completes the game. The date is withheld specifically because it makes the outcome searchable. **Known, accepted limitation:** the research prose itself names dated news events (verified: "a September 9, 2026 market wrap…"), so a determined player can still date a matchup and look up the result. Unavoidable without gutting the research, which IS the product, and tolerable only because practice cannot reach the leaderboard — the only person a cheat misleads is themselves. This must be stated in the UI. Guards: one play per game per player (DB unique index), `result_published` games only, gut → final → validate order enforced, completed games cannot be replayed for a better score. `computeProjectedRank()` (`scoring.ts`) reports where a practice average WOULD sit on the live board using competition ranking — a hypothetical, never stored, and the UI must carry the caveat that practice averages run high because the outcome already happened and the research can be re-read without time pressure. 6 tests. **Client shipped same day:** `/practice` (hub: available games, practice record, projected rank) and `/practice/:id` (the full loop — gut → research → final → timed question → reveal), registered in `App.tsx`, linked from the nav menu for signed-in players. Both routes are `noindex` and excluded from the sitemap (auth-gated, no unique public content) — verified. The hub carries the honest caveat in plain sight: practice doesn't affect leaderboard/streak/qualification, AND that the research still names news from that week so a determined player can find the answer and would only be fooling themselves. The list shows tickers, company names and sector but **never the game date** — the server withholds it. Play resumes where the player left off rather than restarting. Both pages gate on `likelyAuthenticated`, NOT `loading`: gating on `loading` left the page on a spinner forever whenever Clerk failed to resolve, with no escape (reproduced locally, since the local Clerk key is production-only). **Recompute run 2026-09-16 — no-op, by inspection not assumption.** `scripts/recompute-qualification.ts` (dry run by default, `--apply` to write; only ever touches `qualificationStatus`, never scores or games played, and reads the threshold from the shared constant so it can't drift). All 12 `leaderboard_stats` rows already agreed with `isQualified()`: 6 players on 39–43 games (5 bots + Paul) were already `qualified` under the old threshold of 20, and the other 6 sit on 1–5 games so are `pending` either way. **Nobody occupies the 10–19 band**, which is the only band the 20→10 change could have affected — so the stale-status hazard was real in principle but had no victims. Verified row-by-row rather than trusting the "0 rows to change" result, since a broken script looks identical to a clean one. Self-corrects from here: `upsertLeaderboardStat` recalculates on every result publish using the current threshold, so the gap only ever existed for players dormant ACROSS the change. Script kept for any future threshold change. |
| Practice mode: metrics + full analysis missing | **Fixed (2026-09-16)** — Paul played two practice games and reported that only the basic research was shown, with no metrics or advanced analysis to qualify a decision. Correct: `practice.getGame` was already SENDING `researchMetrics` and the full narrative, and `PracticeGame.tsx` rendered neither — it showed `pairingRationale` plus `researchSummary` only, falling back to the full content just when no summary existed. Practice is meant to rehearse the real loop, so a thinner decision basis defeats the point. Now renders the **Key Metrics** table grouped exactly as the live game and archive page group it (The Long Game → Game-Day Setup via `metricGroupInfo`, group headers suppressed for legacy single-group games, `MetricExplanationSheet` per row), plus the same **summary-by-default with a "Show full analysis" toggle** the live game uses. Verified against real data: archived games carry **16 metrics each** and a ~3.5k-char snapshot alongside a ~1.5k-char summary, so both views have real substance. **Caught a bug while wiring it:** `researchMetrics` is stored as an ARRAY of `{label, value}` (see `ResearchMetric`), not a keyed object — the first implementation used `Object.entries()`, which would have rendered array indices ("0", "1", "2") as metric labels. Now uses the same conversion `games.getResearch` does, with a shape guard for older rows. Snapshot is preferred over the live row so an archived game shows the research as it stood on the day. **Corrected again same day** after Paul tested on mobile: the first attempt copied the ARCHIVE page's flat single-column table, which stacks every metric vertically — labels wrapped over three lines, the value column collapsed to nothing, and `whitespace-nowrap` made long values like "Well below 2026 high" overflow the card. The LIVE game uses a two-column comparison (company A beside company B, rows paired by metric, banded by group), which is what practice must mirror. Extracted into new `client/src/components/ResearchMetricsPanel.tsx`, reproducing DailyGame's layout exactly including the ticker-prefix stripping and the split-in-half fallback for older games whose labels lack a ticker. **DailyGame deliberately keeps its own copy:** handover §27 records that this layout "was rebuilt multiple times and is correct and stable", and `/game` is auth-gated behind a production-only Clerk key so it cannot be verified locally after a refactor — the risk of silently breaking the live game outweighed the duplication. Worth extracting there when it can actually be checked. |
| Archived price charts for practice (migration 0017) | **Built 2026-09-16, pending migration.** Practice showed research and metrics but no candlestick chart, because the live chart fetches CURRENT prices — for a past game that shows today's market and, worse, reveals the outcome. Paul suggested archiving chart screenshots; **archiving the DATA is strictly better** — a few KB of JSON instead of images, no headless-browser pipeline, and it renders the real interactive chart through the existing component rather than a flat picture. New nullable `game_research.chartSnapshot` JSON column (migration 0017, one additive `ALTER TABLE ... ADD`), shaped `{capturedAt, asOf, series: {[ticker]: Candle[]}}`, following the established `researchSnapshot`/`metricsSnapshot` pattern. **THE safety property:** candles are DAILY and truncated to end the trading day STRICTLY BEFORE the game date, by pure tested `truncateCandlesBeforeGameDate()` in `db.ts` — a candle dated on the game day IS the result, since its open-versus-close is precisely what the player is asked to predict. Enforced in a tested function rather than a UI rule someone could later loosen; 9 tests cover it including after-the-game-day candles, ordering, the length cap and a malformed date. **Intraday ranges are deliberately unavailable for archived charts** (`ARCHIVED_RANGES` = 1mo/3mo/6mo/1y): the stored series is daily, and a 5-minute or hourly view of the game day would hand over the answer. One stored ~1y daily series backs every offered range by client-side slicing, so no extra fetches. `CandlestickChart` gained an optional `archivedCandles` prop that **skips the live query entirely** when supplied (`enabled: !isArchived`) — there is deliberately NO live fallback, since falling back would show today's prices; games without a snapshot simply hide the chart buttons. Capture runs in `closeAndScoreGame` right after `snapshotResearch`, wrapped in try/catch and **explicitly non-fatal**: an external market-data API must never block scoring, which is what players are actually waiting on. Null OHLC values (halts, gaps) are filtered — an incomplete candle can't be drawn. `scripts/backfill-chart-snapshots.ts` fills historical games (dry run by default, `--apply`, `--limit`, `--force`; 2s between games because the free data tier rate-limits hard; skips games that already have one so failures are safe to retry). Practice shows "Prices up to {asOf} — the day before this matchup, exactly what a live player could see." |
| Practice flow now matches the live game + homepage practice section | **Complete (2026-09-17)** — Paul: practice diverged from the live game in ways that "confuse or annoy the player". Live flow is: gut selection ALONE (instinct only, no information) → research → an explanation of what's next plus an explicit "I've Read the Research" declaration → a DEDICATED final-pick screen → submitting opens a confirmation modal stating the validation rules (one attempt, timed, 20% of score, don't navigate away) → only then the TIMED question. Practice had put the final selection directly beneath the research with no declaration step, and opened the question with no preamble and **no timer**. **`ValidationModal` extracted verbatim from `DailyGame.tsx` (lines 42–287) into `client/src/components/ValidationModal.tsx`, now imported by BOTH pages.** This deliberately differs from the `ResearchMetricsPanel` decision to duplicate: the modal carries scoring consequences (single attempt, timed, worth 20%), so two copies would guarantee exactly the behavioural drift Paul just reported. The move is verbatim — only the `export` keywords and an `ArrowRight` import added — and `XCircle` was dropped from DailyGame's imports as it became unused. **DailyGame was touched, so the live game needs a play-through to confirm.** Practice now also owns none of the answer/option state (the modal does), and `submitFinal` opens the confirm phase rather than jumping to a question screen. **Homepage (new SECTION 6b, between the beta-recruitment section and the final CTA — §20's section list updated):** practice was only reachable from the nav menu. The section leads on the lockout — "The daily game locks when the US market opens — same as backing a horse, once they're out of the gates the odds are settled" (Paul's analogy) — then offers the archive as the thing you CAN play right now, with three cards (no waiting / the full brief / practice only, doesn't touch leaderboard or streak). CTA routes signed-in players to `/practice` and signed-out to the sign-up modal. Gated on `likelyAuthenticated`, consistent with the rest of the page. **Copy rewritten 2026-09-17** after Paul noted a first-time visitor needs more depth: the original said the game "locks when the US market opens", which assumes the reader already knows what that means FOR THEM. Now two short paragraphs — state the situation plainly ("New here, and the trading day has already started? Today's game is locked — once the US market opens, picks close for everyone"), keep the horse-race analogy, then reassure and offer the archive. Paul's draft was longer; compressed at his invitation while keeping every beat: who it's addressed to, that it's locked for the day, the analogy, the reassurance, and where to go. Verified rendering. |
| Perfect-score confetti | **Complete (2026-09-17)** — Paul asked whether a confetti burst on a 100-point game would "scratch that little retention itch" or read as naff. Shipped, on the view that it is fine PROVIDED it stays rare: a perfect game needs a correct prediction AND a correct validation answer inside the 15-second window, so it is genuinely uncommon and reads as earned rather than as participation. Confetti on every result would be naff; this fires only on exactly 100. New `client/src/components/PerfectScoreConfetti.tsx`, wired into BOTH `GameResult.tsx` (live, `myScore.totalScore === 100`) and `PracticeGame.tsx` (`result.dailyScore === 100`) — practice rehearses the real thing, so it earns the same moment. **No dependency:** ~60 lines of canvas rather than a confetti library, since 366KB of dead weight was only just stripped from the bundle. **Honours `prefers-reduced-motion`** — checked inside the effect (so it is correct if the OS setting changes between visits) and returns before any canvas work; verified empirically by forcing the media query and re-mounting, painted pixels 0 vs 19,701 normally. The canvas is `aria-hidden`, `pointer-events-none`, and sits at z-40 — above the page, below `ValidationModal` at z-50. **Retuned 2026-09-17** after Paul reported it covered only the bottom third and vanished quickly — "anticlimactic". Two real bugs, not just tuning. **(1) The physics were per-FRAME, not per-second**, so on a 120Hz ProMotion iPhone every piece got twice the gravity per second: half the height and half the duration. It would have looked correct on a 60Hz screen, which is why it passed local review. Now delta-time based (px/s), with `dt` clamped at 50ms so a backgrounded tab can't resume with one giant step. **(2) The launch angle sign was inverted** in that rewrite, so both launchers fired OUTWARD off the sides — a thin strip of confetti down each edge, which is what the verification screenshots showed. Launch speed is now derived from viewport height (`v = √(2·g·rise)`, rise 0.8–1.25× the screen) so it fills a phone and a desktop alike rather than being tuned to one size, and downward speed is capped at a terminal velocity so pieces flutter down instead of plummeting — that cap is what makes the burst last rather than clearing in two seconds. 150 pieces, 5s, horizontal sway, fade over the final quarter. **Measured, not eyeballed:** sampling the canvas showed the burst reaches 0% from the top (the very top of the screen) at ~2.3s, peaks at ~1,262 painted sample points, and is gone at 4.85s. |
| Locked-game → practice redirect (replaces the homepage practice section) | **Changed (2026-09-17)** — the homepage practice section shipped earlier the same day sat at the bottom of a long scroll, so it only reached visitors who had already scrolled past everything else — the opposite of the newcomer it was written for. Paul: better to hang it off the "Play Today's Game" / "Make Your Pick" CTAs so a locked game opens practice instead. **Implemented one level deeper than asked, at `/game` rather than in each CTA**, which achieves the same capture at the same moment but also covers the bottom-nav tab, direct links and emails, and — importantly — avoids hijacking a player who already picked and is returning to see their own game. `DailyGame` redirects to `/practice?missed=1` when the player is authenticated, `myPick` has resolved, the game is `locked`/`result_published`, AND they have **no gut selection**. A player with a pick is never redirected; signed-out visitors are left alone since `/practice` would only bounce them to the same sign-in prompt. Uses `replace: true` so Back doesn't trap them in a redirect loop. The explanation moved from the homepage onto the practice page as a banner shown only for `?missed=1` — someone who picked Practice from the menu already knows why they're there. SECTION 6b removed from `Home.tsx` (§20 outline updated) along with the icons only it used. Verified both branches: banner present with `?missed=1`, absent without. |
| "While you wait" — alternatives when there's no live game | **Complete (2026-09-17)** — Paul, on the locked-game redirect: what's wrong with offering a returning player who HAS already played the same alternatives, plus the research archive and lessons? Right — they were a dead end too. A player who had submitted saw "Your final selection is locked in. Results will be published after the game closes" and nothing else, for the **seven hours between the 9:30 ET lockout and results after the close**. New `client/src/components/MoreToPlay.tsx`: three cards — practise a past matchup, read the archive, take a lesson — shown on `/game` whenever `step === "submitted" || isLocked` (a locked player with no pick never reaches it, having been redirected to practice already), and again at the foot of `/game/:id/result`, which is the other dead end: result read, tomorrow's game hours away. Deliberately NOT an attempt to extend a session indefinitely — the once-a-day scarcity is doing real work and shouldn't be diluted; the point is only that a player who has finished and wants more shouldn't have to hunt through a menu to discover the archive and learning hub exist. Complements rather than replaces the existing lesson-of-the-day card, which is matchup-specific. |
