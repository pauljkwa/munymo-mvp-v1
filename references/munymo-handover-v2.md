...
| Source attribution for daily matchup | **Code complete, migration NOT yet applied (2026-07-09)** — new nullable `sourceUrl`/`sourceTitle`/`sourcePublisher` columns added to `dailyGames` in `drizzle/schema.ts`, threaded through `admin.endOfDay`'s Zod input, `CurationPayload`, and the curation agent's system prompt/JSON output (it now captures the exact article that gave it the "buzz" signal for the matchup, credited on the game page with a link back to the source). Also wired into `AdminEndOfDay.tsx`'s manual JSON-import/form path so the manual End of Day recovery flow doesn't silently drop attribution. Displayed on `/game/:id` and the archive page under Pairing Rationale. **`npx drizzle-kit generate && drizzle-kit migrate` (`pnpm db:push`) has NOT been run** — needs Paul's written approval per this file's schema-change rule before the columns exist in the live DB. |
| Outbound article click tracking | **Code complete, NOT yet committed/migrated (2026-07-09)** — new `outbound_clicks` table (migration `0014_tan_magma.sql`, generated locally, purely additive) records each click on a daily-matchup source-article link (gameId/userId/publisher/sourceUrl, all nullable). Public `games.recordOutboundClick` mutation fired fire-and-forget from the source links in `DailyGame.tsx`/`ArchiveGame.tsx` (attributes to `ctx.user` when signed in, counts anonymous too). Admin `admin.outboundClickStats` query + a card on the admin dashboard show total clicks + per-publisher breakdown — the "we sent you N readers" ammunition for publisher partnership pitches. Complements the outbound-referral strategy (rel="noopener" + UTM tags). **Awaiting Paul's written approval before `pnpm db:push` / push to main**, per the schema-change rule. |
