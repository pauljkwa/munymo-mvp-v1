...
const CLERK_USER_ID = "user_3FBAkGF3n0W720K6qR27IUuta0h";
...
// ─── Generate two separate Clerk magic links ──────────────────────────────────
// Uses /api/magic wrapper so expired tokens show our custom fallback, not Clerk's error
async function createMagicLink(destination) {
  const res = await fetch("https://api.clerk.com/v1/sign_in_tokens", {
    ...
    body: JSON.stringify({ user_id: CLERK_USER_ID, expires_in_seconds: 86400 }),
  });
  const data = await res.json();
  if (!data.id) throw new Error("Token creation failed: " + JSON.stringify(data));
...
const [resultMagicLink, playMagicLink] = await Promise.all([
  createMagicLink("/game/1/result"),
  createMagicLink("/game"),
]);

console.log("✓ Result link generated");
console.log("✓ Play link generated");

// ─── Build email ──────────────────────────────────────────────────────────────
const winnerTicker = "AMD";
const winnerName   = "Advanced Micro Devices, Inc.";
const loserTicker  = "INTC";
const gameDate     = "2026-06-15";
...
const html = emailWrapper(`
  ...
  <p style="margin:0 0 28px 0;font-size:15px;color:${TEXT_MUTED};">
    <strong style="color:${BRAND_GREEN};">${winnerTicker}</strong> (${winnerName}) outperformed on ${gameDate}.
  ...
  <div style="text-align:center;margin-bottom:8px;">
    ${greenButton(resultMagicLink, "See Yesterday's Result →")}
  </div>
  ...
  <div style="text-align:center;">
    ${greenButton(playMagicLink, "Play Today's Game →")}
  </div>
`);
...
const { data, error } = await resend.emails.send({
  ...
  subject: `You missed it — ${winnerTicker} beat ${loserTicker} on ${gameDate}`,
  html,
});
